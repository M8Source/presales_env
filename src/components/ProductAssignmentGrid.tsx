...
export function ProductAssignmentGrid() {
  const { productAssignments, deleteProductAssignment, loading } = useUserAssignments();
  ...
  const handleDelete = async (id: string) => {
    if (confirm('¿Estás seguro de que quieres eliminar esta asignación?')) {
      await deleteProductAssignment(id);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Asignaciones de Productos</h3>
          ...
          <Card>
            <CardContent className="pt-6">
              <p className="text-center text-muted-foreground">
                No hay asignaciones de productos registradas
              </p>
            </CardContent>
          </Card>
        ) : (
